...
function RCLootCouncilML:AddItem(item, bagged, slotIndex, index)
	index = index or #self.lootTable + 1
	...
	self.lootTable[index] = { -- We want to reserve the index even if we haven't fully loaded the item
		["bagged"]		= bagged,
		["lootSlot"]	= slotIndex,
		["awarded"]		= false,
	...
	local tokenSlot = self.lootTable[index].token
	if tokenSlot then
		if tokenSlot == "Trinket" then
			self.lootTable[index].equipLoc = INVTYPE_TRINKET
		else
			for loc, slot in pairs(addon.INVTYPE_Slots) do
				if slot == tokenSlot then
					self.lootTable[index].equipLoc = loc
				end
			end
		end
	end
	...
	if not name then
		self:ScheduleTimer("Timer", 1, "AddItem", item, bagged, slotIndex, index)
		addon:Debug("Started timer:", "AddItem", "for", item)
	else
		addon:SendMessage("RCMLAddItem", item, index)
	end
end
